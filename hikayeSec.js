    ...
    for (const [colorClass, keywords] of Object.entries(categoryKeywords)) {
        for (const keyword of keywords) {
            if (categoryName.includes(keyword) || categorySlug.includes(keyword)) {
                const iconMap = {
                    'color-pink': 'cat', 'color-light-blue': 'rocket', 'color-purple': 'wand',
                    'color-yellow': 'swords', 'color-green': 'handshake', 'color-cyan': 'tree-palm'
                };
...
function showLoading() {
    ...
    categoryGrid.innerHTML = `
        <div style="grid-column: 1 / -1; text-align: center; padding: 3rem;">
            <div style="width: 50px; height: 50px; border: 3px solid rgba(255, 255, 255, 0.3); border-top: 3px solid #fff; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 1.5rem;"></div>
            <p style="color: rgba(255, 255, 255, 0.9); font-size: 1.1rem; font-weight: 600;">Kategoriler yükleniyor...</p>
        </div>
    `;
...
// API'den kategorileri çeker
async function fetchCategoriesFromAPI() {
    try {
        if (DEBUG_MODE) console.log('📡 localStorage\'da veri yok, API\'den çekiliyor...');
...
// Kategorileri sayfaya render eder
function renderCategories(categories) {
    ...
    const categoryCards = categories.map((category, index) => createCategoryCard(category, index)).join('');
...
async function loadCategories() {
    ...
    try {
        ...
        if (cachedCategories) {
            ...
            if (DEBUG_MODE) console.log('🎉 Kategoriler API\'den başarıyla yüklendi ve cache\'lendi.');
        }
